...
f = @(x,y) x.^3 - sin(x) - log(y);
xvals = linspace(-1.5, 1.25); 
yvals = zeros(size(xvals));  

for j = 1:length(xvals)
   
   yvals(j) = fzero(@(y) f(xvals(j), y), exp((xvals(j))^3-sin(xvals(j))));
...
% As x approaches infinite, x plateaus and becomes flat as it approaches x axis
...
% From the graph, the solution seems to approach y = -1 as x goes to
% infinity. These solutions also intersect the y axis at around -1. All the 
% solutions that can be seen on the
% directional field are solutions with different constant values.
% Solutions cannot cross because if they did there could be more than one
% solution that satisfied one initial condition.
